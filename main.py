# -*- coding: utf-8 -*-
"""
Created on Wed Dec 06 22:19:20 2017
...
GANGLIA_CEREBELLUM_RATIO = 4
if CEREBELLUM == True:
    K = 1. / GANGLIA_CEREBELLUM_RATIO
else:
    K = 1.
    
startPerturbation = 2001
endPerturbation = 2001
...
if __name__ == "__main__":
 ...
 #       print epochAvgAcc 
        ...
        if epochAvgAcc < 100.:    
            T = 1. - (epoch * 0.5/ float(maxEpoch)) #+ 0.1 #* utils.clipped_exp((- epoch * 1.) / float(maxEpoch))
       #     print T
        else:
            T = (1. - epochAvgAcc / 100.)# + 0.1
          #  print epochAvgAcc 
           # print T
        ...
        for trial in xrange(maze.maxTrial):
            ...
            for step in xrange(maxStep):
                ...
                if step > 0:  
                    ...
                    if step > (GANGLIA_CEREBELLUM_RATIO - 1):    
                        if step % GANGLIA_CEREBELLUM_RATIO == 1:  
            ...
            #    wrist.curr3DPos = np.array([0.5,1.,0.5])
                ...
                maze.curr2DPos = utils.conversion2d(utils.change_range(wrist.curr3DPos , 0, 1, -np.pi / 2, np.pi/ 2), perturbation, pertMag)
                    ...
                    if AGENT_VISION == True:
                        ...
                        if FASTLEARNING == True:
                            if perturbation == False:
                                ...
                                bg.compAgentVisionState(utils.change_range(cb.estVision, -1., 1., 0., 1.))
                        else:
                            bg.compAgentVisionState(utils.change_range(maze.curr2DPos, -1., 1., 0., 1.))
                        if GOAL_VISION == True:
                            bg.visionState = np.hstack([bg.agentVisionState, bg.goalVisionState]).copy()
                ...
                if CEREBELLUM == True:
                    if step >0:
                        cb.errorEstVision = utils.change_range(maze.curr2DPos, -1., 1., 0., 1.) - cb.estVision
                      #  print cb.fwdVisionError 
                        cb.trialFwdError += np.sqrt(np.sum(cb.errorEstVision**2))
                    
                    if FASTLEARNING == True:
                        if perturbation == True:
                            if cb.fwdVisionError > 1.:
                                bg.visionState = cb.estVision.copy()
                                bg.currState = np.hstack([bg.wristState, bg.visionState]).copy() 
                ...
                if step > (GANGLIA_CEREBELLUM_RATIO-1):
                    if maze.distance < 0.15:
                        ...
                        if step % GANGLIA_CEREBELLUM_RATIO == 0:
                            bg.spreadCrit()
                    
                    if FASTLEARNING == True:
                        if perturbation == True:
                            if cb.fwdVisionError < cb.prvFwdVisionError:
                                bg.actRew = 0.5
                                bg.currCritOut *= 0.
                        
        
                    if (bg.actRew == 1 or bg.actRew == 0.5 or step % GANGLIA_CEREBELLUM_RATIO == 0):
                        bg.compSurprise()  
                ...
                if TRAINING == True:
                    ...
                    if step > (GANGLIA_CEREBELLUM_RATIO - 1):                        
                        if INTRALAMINAR_NUCLEI == True:  
                            if (step % GANGLIA_CEREBELLUM_RATIO == 0 or bg.actRew == 1):
                                bg.actW += bg.ACT_ETA * (np.outer(bg.prv5State , (K * bg.surp * bg.prvNoise)) + np.outer(bg.prvState, (1-K) * T * (cb.prvOut - bg.prvTrainOut)))
                            
                          #♦  bg.actW += (K * np.outer(bg.prv5State, bg.prvNoise) * bg.surp * bg.ACT_ETA * bg.prvActOut * (1. - bg.prvActOut)) + ((1-K) * np.outer(bg.prvState, (cb.prvOut- bg.prvActOut)) * bg.ACT_ETA * bg.prvActOut * (1. - bg.prvActOut)  * T)
                        else:
                            if (step % GANGLIA_CEREBELLUM_RATIO == 0 or bg.actRew == 1):
                                bg.trainAct()
                        if (step % GANGLIA_CEREBELLUM_RATIO == 0 or bg.actRew == 1):
                    ...
                    if CEREBELLUM == True:
                        cb.trainFwdVision(maze.curr2DPos)
                        if (bg.actRew == 1):
                 ...
                 #           if (step % GANGLIA_CEREBELLUM_RATIO == 0 or bg.actRew == 0.5):
                  #              bg.actW += np.outer(bg.prv5State, bg.prvNoise) * bg.surp * bg.ACT_ETA            
                  #              cb.trainOut = utils.sigmoid(np.dot(cb.w.T, bg.prv5State))
                  #              cb.errorOut = wrist.prvEp - cb.trainOut
                  #              cb.w += cb.cbETA * np.outer(bg.prv5State, cb.errorOut) * bg.surp * cb.trainOut * (1. - cb.trainOut) * T
