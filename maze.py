# -*- coding: utf-8 -*-
"""
Created on Wed Dec 06 22:45:16 2017
...
class Maze8Arm:
    ...
    def init(self, returnGoal, maxStep, maxEpoch):
  ...
  #      self.seArmPol  = geo.Polygon([(6.2,4.5),(8.7,2),(8,1.3), (5.5,3.8)])
   ...
   #     self.centerInt= geo.LinearRing([(5.47, 6.17), (6.17,5.47),(6.17,4.53),(5.47,3.83),(4.53,3.83),(3.83,4.53),(3.83,5.47),(4.53,6.17)])
        ...
        self.maxTrial = len(self.rewPosList)
        
        self.rewPos = np.zeros(2)
        self.distance = 0
        self.curr2DPos = np.zeros(2)
        self.delta2DPos = np.zeros(2)
        self.next2DPos = np.zeros(2)
        
        self.posBuff = np.zeros([2,maxStep])
        self.trajectories = np.zeros([2, maxStep, self.maxTrial, maxEpoch])
    ...
    def setGoal(self, trial):       
        for i in xrange(len(self.rewPosList)):
            if trial % len(self.rewPosList) == i:
                self.rewPos = self.rewPosList[i]
